<script lang="ts">
  ...
  const navigateToByRole = (role: string) => {
    const url = role === 'admin' ? '/lk/admin' : role === 'user' ? '/lk/user' : '/error';
    navigateTo(url);
  };
...
<div class="container">
  ...
  <div class="top-bar">
    ...
    {#if isAuthenticated && isRoleLoaded}
      <button class="profile-btn" on:click={handleProfileClick}>Профиль</button>
    {:else}
      <button class="login-btn" on:click={() => navigateTo('/auntification/selectrole')}>Вход</button>
    {/if}
  </div>
  ...
  {:else}
    <div class="event-cards">
      {#each events as event (event.event_id)}
        ...
        <div class="event-card" on:click={() => handleEventClick(event.event_id)}>
